...
import { Users, Package, TrendingUp, AlertTriangle, Calendar, Activity, CircleOff, Pencil } from "lucide-react"
...
export default async function DashboardPage() {
  try {
    ...
    const { data: todosProdutos } = await supabase
      .from("produtos")
      .select("id, nome, estoque_atual, estoque_minimo, unidade_medida, categoria_id")
      .eq("ativo", true)
      .order("estoque_atual", { ascending: true })

    const produtosBaixoEstoque = todosProdutos?.filter((p) => p.estoque_atual <= p.estoque_minimo) || []
    const totalBaixoEstoque = produtosBaixoEstoque.length

    // ✅ Produtos zerados
    const { count: produtosZerados } = await supabase
      .from("produtos")
    ...
    const produtosAlertaComCategoria =
      ...
      <div className="flex-1 space-y-6 p-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-orange-600">{totalBaixoEstoque}</div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-red-600">{produtosZerados || 0}</div>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
          <Card className="col-span-4">
            ...
            <CardContent>
              {produtosAlertaComCategoria.length > 0 ? (
                <div>
                  {/* Scroll com limite de altura */}
                  <div className="max-h-72 overflow-y-auto pr-1 space-y-3">
                    {produtosAlertaComCategoria.map((produto) => (
                      <div
                        key={produto.id}
